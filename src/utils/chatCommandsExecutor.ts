...
export async function executeCommand(
  ...
  switch (command.type) {
    ...
    case 'delete_category': {
      ...
      return deleteCategory(cat.id).then(() => ` Categoria "${command.name}" removida.`).catch((err: unknown) => ` Erro ao remover: ${err instanceof Error ? err.message : 'desconhecido'}`);
    }
    ...
    case 'delete_recurring': {
      ...
      const bill = recurringBills.find(b => b.name.toLowerCase().includes(command.name.toLowerCase()));
      if (!bill) return ` Conta recorrente "${command.name}" não encontrada.`;
      return deleteRecurringBill(bill.id).then(() => ` Conta recorrente "${bill.name}" removida.`).catch((err: unknown) => ` Erro ao remover: ${err instanceof Error ? err.message : 'desconhecido'}`);
    }
    case 'generate_bills': {
