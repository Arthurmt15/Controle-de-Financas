...
const InstallmentsPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const totalRemainingInst = installments.reduce(
      (s, i) => s + (i.totalInstallments - i.currentInstallment) * Number(i.installmentAmount),
      0
    );
    const totalRemainingDebts = debts.reduce(
      (s, d) => s + (d.totalInstallments - d.currentInstallment) * Number(d.installmentAmount),
      0
    );
    return {
      total: all.length,
  ...
  const nextDueUnified = useMemo(() => {
    ...
    const activeDebts = debts
      .filter((d) => d.currentInstallment < d.totalInstallments)
      .map((debt) => ({
        item: debt,
        type: 'Dívida' as const,
        dueDate: addMonths(debt.startDate, debt.currentInstallment),
      }));
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      daysLabel: nextDueUnified
        ? nextDueUnified.daysUntil === 0
          ? 'vence hoje'
          : nextDueUnified.daysUntil > 0
            ? `em ${nextDueUnified.daysUntil}d`
            : `vencido há ${Math.abs(nextDueUnified.daysUntil)}d`
        : null,
      sub: nextDueUnified
        ? `${nextDueUnified.parcelLabel} • ${formatCurrency(Number(nextDueUnified.amount))}`
        : 'Tudo em dia',
    } as any,
  ];
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-[13.5px] text-muted-foreground max-w-[620px] leading-relaxed">
              Um só lugar para parcelados e dívidas divididas. Ao lançar uma transação como
              dividida, ela aparece automaticamente aqui.
            </p>
          </div>
        ...
        <Card className="border-dashed bg-muted/30">
          <CardContent className="p-3 flex gap-2.5 items-start">
            ...
            <p className="text-[13px] leading-relaxed text-muted-foreground">
              Parcelados e dívidas compartilham a mesma lógica (parcelas, vencimento, progresso).
              Use o filtro <strong>Tipo</strong> para ver só um deles. Transações marcadas como
              divididas viram dívidas automaticamente.
            </p>
          </CardContent>
        </Card>
      </motion.div>
                ...
                {card.label === 'Próximo pagamento' && (card as any).description ? (
                  <div className="min-w-0 flex-1 flex flex-col gap-1">
                    <span className="block text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">
                      {card.label}{' '}
                      <span className="ml-1 inline-flex align-middle text-[9px] font-bold px-1.5 py-0.5 rounded-full border bg-muted text-muted-foreground">
                        {(card as any).typeLabel}
                      </span>
                    </span>
                    <div className="flex items-center gap-2 flex-wrap">
                      <strong className="text-[16px] font-bold tracking-tight leading-none">
                        {card.value}
                      </strong>
                      <span
                        className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full border leading-none ${(card as any).daysUntil !== null && (card as any).daysUntil < 0 ? 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-300 dark:border-red-500/20' : (card as any).daysUntil === 0 ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-500/20' : (card as any).daysUntil !== null && (card as any).daysUntil <= 7 ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-500/10 dark:text-blue-300 dark:border-blue-500/20' : 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20'}`}
                      >
                        {(card as any).daysLabel}
                      </span>
                    </div>
                    <p
                      className="text-[12.5px] font-medium leading-tight truncate text-foreground"
                      title={(card as any).description}
                    >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <strong
                      className="block mt-1.5 text-[18px] font-bold tracking-tight leading-none truncate"
          ...
          <CardContent className="p-3 sm:p-4 flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
            ...
            <Select
              ...
              onValueChange={(v) => {
                ...
                setActiveList(v === 'all' ? 'all' : v === 'installment' ? 'installment' : 'debt');
              }}
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="installment">Só parcelados</SelectItem>
                <SelectItem value="debt">Só dívidas</SelectItem>
              </SelectContent>
            </Select>
            <Badge
              variant="outline"
              className="justify-center lg:justify-start py-1.5 px-3 rounded-full text-xs font-medium whitespace-nowrap"
            >
      ...
      <div className="mt-5 flex justify-center">
        <div className="inline-flex p-1 rounded-full bg-muted border gap-1">
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${activeList === 'all' ? 'bg-card shadow-sm border text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Tudo ({metrics.total})
          </button>
          <button
            onClick={() => setActiveList('installment')}
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${activeList === 'installment' ? 'bg-card shadow-sm border text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Parcelados ({installments.length})
          </button>
          <button
            onClick={() => setActiveList('debt')}
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${activeList === 'debt' ? 'bg-card shadow-sm border text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
