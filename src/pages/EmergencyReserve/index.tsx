...
const EmergencyReservePage: React.FC = () => {
  ...
  const suggestedGoal = useMemo(() => {
    ...
    const avg = monthsCount > 0 ? totalExpense / monthsCount : transactions.filter((t) => t.type === 'expense').reduce((s, t) => s + t.amount, 0) / 3 || 0;
    return Math.round(avg * 6);
  }, [transactions]);

  const progress = reserve && reserve.goalAmount > 0 ? Math.min(100, (reserve.currentAmount / reserve.goalAmount) * 100) : 0;
  const remaining = reserve ? Math.max(0, reserve.goalAmount - reserve.currentAmount) : 0;
  const isCompleted = reserve ? reserve.currentAmount >= reserve.goalAmount && reserve.goalAmount > 0 : false;
  ...
  const handleDeposit = async () => {
    ...
    if (!val || val <= 0) { setError('Informe um valor para depositar'); return; }
    setError(null);
    try {
      await deposit(val);
      setDepositInput('');
  ...
  const handleWithdraw = async () => {
    ...
    if (!val || val <= 0) { setError('Informe um valor para sacar'); return; }
    setError(null);
    try {
      await withdraw(val);
      setWithdrawInput('');
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 py-6">
        <Card className="rounded-2xl border-dashed"><CardContent className="p-12 text-center text-muted-foreground">Carregando reserva...</CardContent></Card>
      </div>
    );
  }
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
          <div className="min-w-0">
            <h1 className="text-[28px] sm:text-[30px] font-bold tracking-tight leading-none bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent flex items-center gap-2">
              <span className="w-9 h-9 rounded-xl bg-emerald-500/10 border border-emerald-500/10 flex items-center justify-center text-emerald-600"><PiggyBank size={18} /></span>
              Reserva de Emergência
            </h1>
            <p className="mt-2 text-[13.5px] text-muted-foreground max-w-[560px] leading-relaxed">
              Sua rede de segurança financeira — ideal é 6 meses de despesas. Acompanhe progresso e movimente quando precisar.
            </p>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.08 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 mt-5">
        ...
        <Card className="relative overflow-hidden rounded-2xl border bg-card shadow-sm">
          ...
          <CardContent className="p-[18px] flex items-center gap-3.5">
            <span className="w-[42px] h-[42px] flex items-center justify-center rounded-xl border bg-violet-50 border-violet-100 text-violet-600 dark:bg-violet-500/10 dark:border-transparent"><Wallet size={16} /></span>
            <div className="min-w-0 flex-1">
              <span className="block text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">Meta definida</span>
              <strong className="block mt-1.5 text-[18px] font-bold tracking-tight">{reserve ? formatCurrency(reserve.goalAmount) : '—'}</strong>
              <span className="mt-1.5 inline-block text-[11px] font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{reserve ? 'Editável' : 'Crie sua meta'}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden rounded-2xl border bg-card shadow-sm">
          ...
          <CardContent className="p-[18px] flex items-center gap-3.5">
            <span className="w-[42px] h-[42px] flex items-center justify-center rounded-xl border bg-emerald-50 border-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:border-transparent"><PiggyBank size={16} /></span>
            <div className="min-w-0 flex-1">
              <span className="block text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">Guardado</span>
              <strong className="block mt-1.5 text-[18px] font-bold tracking-tight">{reserve ? formatCurrency(reserve.currentAmount) : formatCurrency(0)}</strong>
              <span className="mt-1.5 inline-block text-[11px] font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{reserve ? `${progress.toFixed(1)}% da meta` : 'Comece agora'}</span>
            </div>
          </CardContent>
        </Card>

        <Card className={`relative overflow-hidden rounded-2xl border bg-card shadow-sm ${isCompleted ? 'ring-1 ring-emerald-500/20' : ''}`}>
          <span className={`absolute left-0 top-0 bottom-0 w-[3px] ${isCompleted ? 'bg-emerald-500' : 'bg-gradient-to-b from-amber-500 to-orange-500'}`} />
          <CardContent className="p-[18px] flex items-center gap-3.5">
            <span className={`w-[42px] h-[42px] flex items-center justify-center rounded-xl border ${isCompleted ? 'bg-emerald-50 border-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:border-transparent' : 'bg-amber-50 border-amber-100 text-amber-600 dark:bg-amber-500/10 dark:border-transparent'}`}>
              {isCompleted ? <ShieldCheck size={16} /> : <TrendingUp size={16} />}
            </span>
            <div className="min-w-0 flex-1">
              <span className="block text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">{isCompleted ? 'Completa!' : 'Falta guardar'}</span>
              <strong className="block mt-1.5 text-[18px] font-bold tracking-tight">{reserve ? formatCurrency(remaining) : formatCurrency(suggestedGoal)}</strong>
              <span className="mt-1.5 inline-block text-[11px] font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{isCompleted ? 'Meta batida' : 'Para atingir a meta'}</span>
            </div>
          </CardContent>
        </Card>
      </motion.div>
      ...
      {error && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4">
          <div className="flex items-center justify-between gap-3 px-4 py-3 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm dark:bg-red-500/10 dark:border-red-500/20 dark:text-red-400">
            ...
            <Button size="sm" variant="ghost" className="rounded-xl shrink-0 h-7 px-2 text-xs" onClick={() => setError(null)}>Dispensar</Button>
          </div>
        </motion.div>
      )}
      ...
      <div className="mt-5 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-4">
            ...
            {!reserve ? (
              ...
              <>
                <div className="flex items-center justify-between">
                  <h3 className="text-[15px] font-semibold flex items-center gap-2"><PiggyBank className="h-4 w-4 text-emerald-600" /> Progresso</h3>
                  <Badge variant={isCompleted ? 'default' : 'secondary'} className={`rounded-full ${isCompleted ? 'bg-emerald-600' : ''}`}>{progress.toFixed(1)}%</Badge>
                </div>
                <div className="h-3 w-full bg-muted rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-500 ${isCompleted ? 'bg-emerald-500' : 'bg-gradient-to-r from-emerald-500 to-teal-500'}`} style={{ width: `${progress}%` }} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="rounded-xl border bg-muted/30 p-3 text-center">
                    <p className="text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">Guardado</p>
                    <p className="text-[15px] font-bold mt-1">{formatCurrency(reserve.currentAmount)}</p>
                  </div>
                  <div className="rounded-xl border bg-muted/30 p-3 text-center">
                    <p className="text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">Falta</p>
                ...
                {isCompleted && (
                  <div className="rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-300 p-3 flex items-center gap-2 text-sm">
                    <ShieldCheck className="h-4 w-4" /> Parabéns! Sua reserva está completa. Mantenha para imprevistos.
                  </div>
                )}
              </>
            )}
        ...
        <Card className="rounded-2xl border shadow-sm overflow-hidden">
          <CardContent className="p-6 space-y-5">
            {!reserve ? (
              <p className="text-sm text-muted-foreground text-center py-6">Crie uma reserva para habilitar depósitos e saques.</p>
            ) : (
              <>
                <div className="space-y-2">
                  <Label className="text-sm flex items-center gap-1.5"><Plus className="h-3.5 w-3.5 text-emerald-600" /> Depositar</Label>
                  <div className="flex gap-2">
                    <Input placeholder="0,00" value={depositInput} onChange={(e) => setDepositInput(e.target.value)} className="rounded-xl" />
                    <Button onClick={handleDeposit} className="rounded-xl bg-emerald-600 hover:bg-emerald-700">Depositar</Button>
                  </div>
                </div>

                <div className="space-y-2">
                  <Label className="text-sm flex items-center gap-1.5"><Minus className="h-3.5 w-3.5 text-red-500" /> Sacar</Label>
                  <div className="flex gap-2">
                ...
                <div className="pt-3 border-t space-y-2">
                  <Label className="text-sm flex items-center gap-1.5"><Edit3 className="h-3.5 w-3.5 text-muted-foreground" /> Editar meta</Label>
                  {!isEditingGoal ? (
                    <div className="flex gap-2">
                      <div className="flex-1 px-3 py-2 rounded-xl border bg-muted text-sm font-medium">{formatCurrency(reserve.goalAmount)}</div>
                      <Button variant="outline" onClick={() => { setIsEditingGoal(true); setGoalInput(String(reserve.goalAmount)); }} className="rounded-xl">Editar</Button>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <Input value={goalInput} onChange={(e) => setGoalInput(e.target.value)} className="rounded-xl" autoFocus />
                      <Button onClick={handleUpdateGoal} className="rounded-xl">Salvar</Button>
                      <Button variant="ghost" onClick={() => setIsEditingGoal(false)} className="rounded-xl">Cancelar</Button>
                    </div>
                  )}
                  <Button variant="ghost" size="sm" className="rounded-full text-xs" onClick={() => setGoalInput(String(suggestedGoal))}>Sugerir {formatCurrency(suggestedGoal)} (6x)</Button>
                </div>

                <div className="pt-2">
                  <Button variant="ghost" size="sm" onClick={remove} className="w-full rounded-xl text-red-600 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-500/10">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4">
            <Card className="rounded-2xl border-dashed bg-muted/20">
              <CardContent className="p-4 flex gap-3">
                <span className="p-2 rounded-xl bg-sky-500/10 text-sky-600 h-fit"><Info className="h-4 w-4" /></span>
                <div className="text-sm leading-relaxed text-muted-foreground">
                  <p><strong className="text-foreground">Dica:</strong> especialistas recomendam 3 a 6 meses de despesas. Com base nos seus últimos gastos, o ideal é <strong className="text-foreground">{formatCurrency(suggestedGoal)}</strong>.</p>
                  <p className="mt-1">Deposite quando receber e só saque em real emergência. Acompanhe o progresso no Dashboard em breve.</p>
                </div>
              </CardContent>
            </Card>
          </motion.div>
