...
import { LayoutDashboard, Receipt, BarChart3, Settings, CreditCard, CalendarClock, Search } from 'lucide-react';
...
export const CommandPalette: React.FC<{ open: boolean; onOpenChange: (o: boolean) => void }> = ({ open, onOpenChange }) => {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh] p-4 bg-black/40 backdrop-blur-sm" onClick={() => onOpenChange(false)}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg rounded-2xl border bg-popover text-popover-foreground shadow-2xl overflow-hidden">
        <Command className="p-0">
          <div className="flex items-center gap-2 px-3 border-b">
            <Search size={16} className="text-muted-foreground" />
            <Command.Input placeholder="Ir para..." className="flex-1 h-11 bg-transparent outline-none text-sm placeholder:text-muted-foreground" autoFocus />
          </div>
          <Command.List className="p-2 max-h-72 overflow-auto">
            <Command.Empty className="p-4 text-sm text-muted-foreground text-center">Nenhum resultado.</Command.Empty>
