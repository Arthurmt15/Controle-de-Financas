...
import { BarChart3, TrendingUp, TrendingDown, Wallet, Sparkles, ArrowUpRight, Layers } from 'lucide-react';
...
const MonthlyChart: React.FC<MonthlyChartProps> = ({ transactions, height = 300 }) => {
  ...
  const stats = useMemo(() => {
    ...
    const trend = prev && Math.abs(prev.saldo) > 0 ? ((last.saldo - prev.saldo) / Math.abs(prev.saldo)) * 100 : 0;
    ...
    return (
      <Card className="rounded-2xl border-dashed bg-muted/20 overflow-hidden">
        <CardContent className="p-8 flex flex-col items-center justify-center gap-3 text-muted-foreground text-center">
          ...
          <p className="text-xs max-w-[260px] leading-relaxed">Adicione transações e veja sua evolução com barras + saldo em um visual novo e unificado.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="rounded-2xl border bg-card/95 backdrop-blur-xl shadow-xl p-3.5 min-w-[210px]">
        <p className="text-[11px] font-semibold tracking-widest uppercase text-muted-foreground mb-2.5">{row.full}</p>
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-4">
            <span className="flex items-center gap-1.5 text-xs font-semibold"><span className="h-2 w-2 rounded-full bg-violet-500" /> {mode === 'acumulado' ? 'Acumulado' : 'Saldo'}</span>
            <span className={`text-sm font-bold ${ (mode === 'acumulado' ? row.acumulado : row.saldo) >= 0 ? 'text-violet-600 dark:text-violet-400' : 'text-red-600'}`}>
    ...
    <div className="w-full min-w-0 overflow-hidden">
      ...
      <div className="flex flex-col gap-3 mb-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl border bg-emerald-50/70 dark:bg-emerald-500/10 px-3 py-2 flex flex-col gap-0.5">
            <span className="text-[10px] font-semibold tracking-widest uppercase text-emerald-700/70 dark:text-emerald-300/80">Entradas 12m</span>
            <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 truncate">{formatCurrency(stats.totalEntradas)}</span>
          </div>
          <div className="rounded-xl border bg-red-50/70 dark:bg-red-500/10 px-3 py-2 flex flex-col gap-0.5">
            <span className="text-[10px] font-semibold tracking-widest uppercase text-red-700/70 dark:text-red-300/80">Saídas 12m</span>
            <span className="text-xs font-bold text-red-700 dark:text-red-300 truncate">{formatCurrency(stats.totalSaidas)}</span>
          </div>
          <div className="rounded-xl border bg-violet-50 dark:bg-violet-500/10 px-3 py-2 flex flex-col gap-0.5">
            <span className="text-[10px] font-semibold tracking-widest uppercase text-violet-700/70 dark:text-violet-300/80">Saldo total</span>
            <span className={`text-xs font-bold truncate ${stats.saldoTotal >= 0 ? 'text-violet-700 dark:text-violet-300' : 'text-red-700'}`}>{formatCurrency(stats.saldoTotal)}</span>
          </div>
        </div>
      </div>

      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="rounded-2xl border bg-card/50 backdrop-blur p-2 sm:p-3">
        <ResponsiveContainer width="100%" height={height}>
          <ComposedChart data={monthlyData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }} barGap={6} barCategoryGap="20%">
            <defs>
            ...
            {mode === 'acumulado' && <Area type="monotone" dataKey="acumulado" fill="url(#anaAcum)" stroke="none" />}
            <Bar dataKey="entradas" name="Entradas" fill="url(#anaEntradas)" radius={[8, 8, 0, 0]} maxBarSize={26} />
            <Bar dataKey="saidas" name="Saídas" fill="url(#anaSaidas)" radius={[8, 8, 0, 0]} maxBarSize={26} />
            {mode === 'fluxo' ? (
              <Line type="monotone" dataKey="saldo" stroke="#8b5cf6" strokeWidth={2.4} dot={{ r: 3.2, fill: '#8b5cf6', stroke: 'white', strokeWidth: 1.8 }} activeDot={{ r: 5, fill: '#8b5cf6', stroke: 'white', strokeWidth: 2 }} />
            ) : (
              <Line type="monotone" dataKey="acumulado" name="Acumulado" stroke="#8b5cf6" strokeWidth={2.4} strokeDasharray="0" dot={{ r: 3, fill: '#8b5cf6', stroke: 'white', strokeWidth: 1.8 }} activeDot={{ r: 5, fill: '#8b5cf6', stroke: 'white', strokeWidth: 2 }} />
            )}
          </ComposedChart>
        </ResponsiveContainer>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mt-2 pt-2.5 border-t text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500" /> Entradas</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-500" /> Saídas</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-3 h-0.5 rounded-full bg-violet-500" /> {mode === 'acumulado' ? 'Acumulado' : 'Saldo do mês'}</span>
        </div>
      </motion.div>
      ...
      <div className="mt-3 grid grid-cols-2 lg:grid-cols-4 gap-2">
        ...
        <div className="rounded-xl border bg-muted/30 px-3 py-2.5 flex flex-col gap-0.5">
          ...
          <span className="text-xs font-bold flex items-center gap-1"><ArrowUpRight className="h-3 w-3 text-emerald-500" /> {stats.best.full} • {formatCurrency(stats.best.saldo)}</span>
        </div>
        <div className="rounded-xl border bg-muted/30 px-3 py-2.5 flex flex-col gap-0.5">
          <span className="text-[11px] font-semibold tracking-widest uppercase text-muted-foreground">Pior mês</span>
          <span className="text-xs font-bold flex items-center gap-1"><TrendingDown className="h-3 w-3 text-red-500" /> {stats.worst.full} • {formatCurrency(stats.worst.saldo)}</span>
        </div>
        <div className="rounded-xl border bg-violet-50 dark:bg-violet-500/10 border-violet-100 dark:border-violet-500/20 px-3 py-2.5 flex items-center gap-2 text-violet-700 dark:text-violet-300">
          ...
          <span className="text-xs font-medium leading-tight">{mode === 'acumulado' ? 'Área roxa = evolução do patrimônio' : 'Linha roxa = saldo mensal'}</span>
        </div>
      </div>
    </div>
  );
