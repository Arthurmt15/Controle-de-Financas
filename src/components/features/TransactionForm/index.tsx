...
const TransactionForm: React.FC<TransactionFormProps> = ({
  ...
  const previewInstallmentAmount =
    formData.creationType === 'parcelado' && parsedAmount > 0 && parsedInstallments > 1
      ? parsedAmount / parsedInstallments
      : 0;
  const parsedDebtInstallments = parseInt(formData.debtInstallments, 10);
  const previewDebtAmount =
    formData.creationType === 'dividida' && parsedAmount > 0 && parsedDebtInstallments > 1
      ? parsedAmount / parsedDebtInstallments
      : 0;
      ...
      if (editingTransaction) {
        ...
        if (formData.creationType === 'parcelado' && parsedInstallments > 1 && parsedAmount > 0) {
          try {
            ...
            console.error('Falhou ao criar parcelado:', err);
          }
        }
        if (formData.creationType === 'dividida' && parsedAmount > 0) {
          try {
            ...
            console.error('Falhou ao criar dívida:', err);
          }
        }
      }
      setFormData({
    ...
    >
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {formData.creationType === 'parcelado'
              ? 'Será criada em Transações + Parcelados'
              : formData.creationType === 'dividida'
                ? 'Será criada em Transações + Dívidas'
                : 'Preencha e salve para registrar'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          >
            <SelectTrigger
              className={`rounded-xl h-10 ${errors.categoryId ? 'border-red-500 focus:ring-red-500' : ''}`}
            >
            ...
            <CardContent className="p-0">
              <div className="px-4 pt-4 pb-2">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Escolha onde este lançamento deve aparecer. Mesma lógica de parcelas.
                </p>
              </div>
              <div className="px-2 pb-2">
                <div className="grid grid-cols-3 gap-1.5 p-1 bg-muted/50 rounded-xl">
                  <Button
                    type="button"
                    variant={formData.creationType === 'none' ? 'default' : 'ghost'}
                    className={`rounded-lg h-9 text-xs font-semibold ${formData.creationType === 'none' ? 'shadow-sm bg-card border text-foreground' : ''}`}
                    onClick={() => setFormData((p) => ({ ...p, creationType: 'none' }))}
                  >
                    Só transação
                  </Button>
                  <Button
                    type="button"
                    variant={formData.creationType === 'parcelado' ? 'default' : 'ghost'}
                    className={`rounded-lg h-9 text-xs font-semibold gap-1 ${formData.creationType === 'parcelado' ? 'shadow-sm' : ''}`}
                    onClick={() => setFormData((p) => ({ ...p, creationType: 'parcelado' }))}
                  >
                  ...
                  <Button
                    type="button"
                    variant={formData.creationType === 'dividida' ? 'default' : 'ghost'}
                    className={`rounded-lg h-9 text-xs font-semibold gap-1 ${formData.creationType === 'dividida' ? 'shadow-sm bg-amber-500 hover:bg-amber-600 text-white' : ''}`}
                    onClick={() => setFormData((p) => ({ ...p, creationType: 'dividida' }))}
                  >
              ...
              {formData.creationType === 'parcelado' && (
                <div className="px-4 pb-4 pt-2 space-y-2 border-t bg-violet-50/40 dark:bg-violet-500/5">
                  <Label htmlFor="totalInstallments" className="text-sm flex items-center gap-1.5">
                  ...
                  {previewInstallmentAmount > 0 && (
                    <Badge
                      variant="outline"
                      className="rounded-full px-3 py-1 text-xs font-medium gap-1 border-violet-200 text-violet-700 dark:border-violet-500/20 dark:text-violet-300"
                    >
                      <Layers className="h-3 w-3" />
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Aparecerá em <strong>Parcelados & Dívidas</strong> como Parcelado.
                  </p>
                </div>
              )}
              {formData.creationType === 'dividida' && (
                <div className="px-4 pb-4 pt-2 space-y-2 border-t bg-amber-50 dark:bg-amber-500/5">
                  <Label htmlFor="debtInstallments" className="text-sm flex items-center gap-1.5">
                  ...
                  {parsedAmount > 0 && (
                    <Badge
                      variant="outline"
                      className="rounded-full px-3 py-1 text-xs font-medium gap-1 border-amber-200 text-amber-700 dark:border-amber-500/20 dark:text-amber-400"
                    >
                      <HandCoins className="h-3 w-3" />
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Aparecerá em <strong>Parcelados & Dívidas</strong> como Dívida dividida.
                  </p>
                </div>
              )}
            </CardContent>
